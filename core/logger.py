...
class RevuexLogger:
    ...
    def log_scanner_complete(self, scanner_name, findings_count=0):
        ...
        self.logger.info(f"{Colors.GREEN}[SCANNER COMPLETE]{Colors.END} {scanner_name} - {findings_count} findings in {activity['duration']:.2f}s")
        self.audit_logger.info(f"SCANNER_COMPLETE: {scanner_name} | Findings: {findings_count} | Duration: {activity['duration']:.2f}s")
    ...
    def log_phase_complete(self, phase_name, summary=""):
        if phase_name in self.performance['phase_start_times']:
            ...
            self.logger.info(f"{Colors.BOLD}{Colors.GREEN}[PHASE COMPLETE]{Colors.END} {phase_name} - {duration:.2f}s {summary}")
            self.audit_logger.info(f"PHASE_COMPLETE: {phase_name} | Duration: {duration:.2f}s | {summary}")
    ...
    def log_vulnerability(self, vuln_type, severity, target, scanner=None):
        ...
        self.logger.info(f"{color}[VULNERABILITY]{Colors.END} {severity.upper()} - {vuln_type} on {target}{scanner_info}")
        self.audit_logger.info(f"VULNERABILITY: {severity.upper()} | Type: {vuln_type} | Target: {target} | Scanner: {scanner or 'Unknown'}")
    ...
    def log_exploit_start(self, exploit_type, target):
        self.logger.info(f"{Colors.YELLOW}[EXPLOIT START]{Colors.END} {exploit_type} on {target}")
    ...
    def log_exploit(self, exploit_type, target, success, details=""):
        ...
        self.logger.info(f"{color}[EXPLOIT {status}]{Colors.END} {exploit_type} on {target} {details}")
        self.audit_logger.info(f"EXPLOIT: {status} | Type: {exploit_type} | Target: {target} | Details: {details}")
