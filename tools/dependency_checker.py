...
class DependencyChecker:
    ...
    def _detect_libraries(self):
        ...
        for lib_name, lib_info in self.library_patterns.items():
            ...
            if matches:
                version = matches[0]
                self.detected_libraries.append({'name': lib_name, 'version': version})
            ...
            if lib_name in self.library_patterns:
                ...
                if version in vulnerable_versions:
                    ...
                    vuln = {
                        ...
                        'version': version,
                        'vulnerability': issue,
                        'poc': f"Target {lib_name} {version} is vulnerable to {issue}",
                        'remediation': [f'Update {lib_name} immediately', 'Use Snyk or npm audit'],
    ...
    def _is_outdated(self, lib_name: str, version: str) -> bool:
        ...
        outdated_thresholds = {
            'jquery': '3.5.0', 'angular': '12.0.0', 'react': '17.0.0',
            'vue': '3.0.0', 'bootstrap': '5.0.0', 'lodash': '4.17.21', 'moment': '2.29.0'
        }
